...
func UploadFile(c *gin.Context) {
    ...
    // Configure B2 client only if credentials are provided
    if config.AppConfig.B2KeyID != "" && config.AppConfig.B2AppKey != "" {
        cfg, err := awsConfig.LoadDefaultConfig(context.Background(),
            awsConfig.WithCredentialsProvider(
                credentials.NewStaticCredentialsProvider(
                    config.AppConfig.B2KeyID, 
                    config.AppConfig.B2AppKey, 
                    "",
                ),
            ),
        ...
        s3Client := s3.NewFromConfig(cfg, func(o *s3.Options) {
            o.BaseEndpoint = aws.String(config.AppConfig.B2Endpoint)
        })

        // Upload to B2
        _, err = s3Client.PutObject(context.Background(), &s3.PutObjectInput{
            Bucket:      aws.String(config.AppConfig.B2BucketName),
